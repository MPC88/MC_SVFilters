    ...
    class CraftingFilter
    {
        ...
        private static bool IsBPFiltered(Blueprint item)
        {
            ...
            if (item.itemType == Main.itemTypeShip)
            {
                ...
                if (smd != null)
                {
                    if (Lang.Get(0, (int)(330 + smd.shipRole)).ToLower().Contains(craftingFilterInput.text.ToLower()))
                    {
                        if (Main.cfgDebug.Value) Main.log.LogInfo("Allowed bp: " + name + " as ship with role: " + Lang.Get(0, (int)(330 + smd.shipRole)));
                        return false;
                    }

                    if (Lang.Get(5, (int)(100 + smd.shipClass)).ToLower().Contains(craftingFilterInput.text.ToLower()))
                    {
                        if (Main.cfgDebug.Value) Main.log.LogInfo("Allowed bp: " + name + " as ship with class: " + Lang.Get(5, (int)(100 + smd.shipClass)));
                        return false;
                    }
                }
            }
